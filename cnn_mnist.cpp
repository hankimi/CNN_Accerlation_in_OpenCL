...
// function to generate a random float between -1 and 1 for kernel
...
void initialise_weights_kernel(int randoms) {
    // 1) initialize weights in the kernel style
    int id = 0;
    unsigned random_idx = 0;
    for (int i = 0; i < 5; i++) {
        for (int j = 0; j < 28; j++) {
            for (int k = 0; k < 28; k++) {
                if (j < 7 && k < 7) {
                    conv_w[i][j][k] = kernel_RNG(randoms + id + random_idx);
                    random_idx++;
                }
            }
        }
    ...
    for (int i = 0; i < 5; i++) {
        for (int j = 0; j < 28; j++) {
            for (int k = 0; k < 28; k++) {
                conv_b[i][j][k] = kernel_RNG(randoms + id + random_idx);
                random_idx++;
            }
        }
    }
    ...
    for (int i = 0; i < 980; i++) {
        for (int j = 0; j < 120; j++) {
            dense_w[i][j] = kernel_RNG(randoms + id + random_idx);
            random_idx++;
        }
    }
    for (int i = 0; i < 120; i++) {
        dense_b[i] = kernel_RNG(randoms + id + random_idx);
        random_idx++;
    }

    for (int i = 0; i < 120; i++) {
        for (int j = 0; j < 10; j++) {
            dense_w2[i][j] = kernel_RNG(randoms + id + random_idx);
            random_idx++;
        }
    }
    for (int i = 0; i < 10; i++) {
        dense_b2[i] = kernel_RNG(randoms + id + random_idx);
        random_idx++;
        //printf("dense_b2[%d]: %0.3f\n", i, dense_b2[i]);
    }
}
...
int main() {
    read_test_data();
    read_train_data();
    const unsigned randoms = 1234567890;
    initialise_weights_kernel(randoms);
    //initialise_weights();
    ...
    for (int i = 0; i < epoch; i++) {
        ...
        for (int j = 0; j < batch_size; j++) {
            num = rand() % 60000;
            // random_count++;
            // if (random_count <= 10) printf("random number: %d\n", num);
            int img[32][32];
            int vector_y[10];
            give_y(label_train[num], vector_y);
            give_img(data_train[num], img);
            /* test if img is the same
             if (i == 0 && j == 0) {
                printf("img = %d\n", img[194 / 32][194 % 32]);
            } */
            // part to accelerate using OpenCL on GPGPU
            /* what data in memory to send to GPU?
            1) array: img, conv_w, conv_b, dense_w, dense_b, dense_w2, dense_b2; vector_y 
            2) variable: eta (fixed, no need to send)
            */
            forward_pass(img);
            backward_pass(dense_softmax, vector_y, img);
            update_weights();
            /* what data to receive from GPU?
            1) array: conv_w, conv_b, dense_w, dense_b, dense_w2, dense_b2
            */
        }
    }
